...
def lean_args(sequence, reading_frames, i, j) -> tuple :
    global lean_buffer

    lean_key : int = len(lean_buffer)
...
class lean_call:
    ...
    def __call__(self, lean_key, reading_frames, i, j) -> list :
        global lean_buffer
        ...
        for frame, n, frequences in results:
            lean_frequences : defaultdict = defaultdict(int)
...
def count_frequencies(sequence, reading_frames, i, j) -> list :
    frames : tuple = tuple(
    ...
    short_frame_frequences : tuple = frequences_mask_list[1:]

    mono_nucleotides : list = []
    frame_tail : int = len(frames) - 1
    if frame_tail >= 0 and frames[frame_tail] == 1:
        freq : defaultdict = frequences_mask_list[frame_tail][0]
        worklist = sequence[i:j]
        len_before : int = len(worklist)
        while len_before > 0:
            n = worklist[0:1]
            worklist = worklist.translate(None, n)
            len_after : int = len(worklist)
    ...
    short_frame_frequences : tuple = short_frame_frequences[:frame_tail]
...
def read_sequence(file, header, translation) -> (bytearray | Any) :
    for line in file:
        if line[0] == ord('>'):
...
def lookup_frequency(results, frame, bits) -> tuple :
    n : Literal = 1
    frequency : Literal = 0
...
def main() -> None:
    translation : bytes = bytes.maketrans(b'GTCAgtca',
    ...
    sequence : bytearray = read_sequence(stdin.buffer, b'THREE', translation)

    mono_nucleotides : tuple = ('G', 'A', 'T', 'C')
    di_nucleotides : tuple = tuple(n + m
    ...
    partitions : list = [len(sequence) * i // n for i in range(n+1)]
    count_jobs : list = [
        (sequence, reading_frames, partitions[i], partitions[i + 1])
            for i in range(len(partitions) - 1)]

    if n == 1:
        results : list = list(chain(*starmap(count_frequencies, count_jobs)))
    else:
        lean_jobs : list = list(starmap(lean_args, count_jobs))
        with Pool() as pool:
            async_results = pool.starmap_async(
                lean_call(count_frequencies), lean_jobs)
            results : list = list(chain(*async_results.get()))
