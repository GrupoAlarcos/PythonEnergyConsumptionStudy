...
def permutations(n: int, start: int, size: int) -> 'Generator[bytearray, None, None]':
    p: bytearray = bytearray(range(n))
    count: bytearray = bytearray(n)
...
def alternating_flips_generator(n: int, start: int, size: int) -> 'Generator[int, None, None]':
    maximum_flips: int = 0
    alternating_factor: int = 1
...
def task(n: int, start: int, size: int) -> 'Tuple[int, int]':
...
def fannkuch(n: int) -> None:
    if n < 0:
        ...
        task_count: int = cpu_count()
        total: int = factorial(n)
        task_size: int = (total + task_count - 1) // task_count
        ...
        task_args: 'List[Tuple[int, int, int]]' = [(n, i * task_size, task_size) for i in range(task_count)]

        checksums: Tuple; maximums: Tuple
        if task_count > 1:
            with Pool() as pool:
